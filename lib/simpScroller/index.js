...
var simpScroller = (function() {
	...
	var _scroller = function(container, direction, params) {
		...
		// 事件
		var pos = {};
		container.addEventListener(_event.start[0], function(event) {
        ...
        container.addEventListener(_event.start[1], function(event) {
            sizeContainerWithScroll = this["scroll" + Size];
            pos[pageKey] = event.touches? event.touches[0][pageKey]: event[pageKey];
            pos[key] = this["scroll" + Key];
            document.moveFollow = true;
        ...
        container.addEventListener(_event.move[0], function(event) {
        ...
        container.addEventListener(_event.move[1], function(event) {
            if ( _upSupportTouch == true || document.moveFollow == true) {
                // touch设备或有可移动标志
                this["scroll" + Key] = pos[key] + (pos[pageKey] - (event.touches? event.touches[0][pageKey]: event[pageKey]));
		...
		container.addEventListener(_event.end[0], function(event) {
			scroller && (scroller.style.opacity = 0);
		});
        container.addEventListener(_event.end[1], function(event) {
            scroller && (scroller.style.opacity = 0);
        });
	...
	return function(container, options) {
		...
		if(container.isScroll){
			container.getElementsByTagName('div')[0].scrollTop=0;
            container.getElementsByTagName('div')[0].scrollLeft=0;
            return false;
		}else{
